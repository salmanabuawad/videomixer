    ...
    <div className="min-h-screen flex items-center justify-center bg-theme-content px-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-theme-text-primary mb-2">Video Mixer</h1>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-theme-card-border">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label htmlFor="username" className="block text-sm font-medium text-theme-text-primary mb-2">Username</label>
              <input
                id="username" type="text" value={username}
                onChange={e => setUsername(e.target.value)}
                required disabled={loading} autoComplete="username"
                placeholder="Enter username" className="input-base"
              />
            </div>

            <div>
              <label htmlFor="password" className="block text-sm font-medium text-theme-text-primary mb-2">Password</label>
              <div className="relative">
                <input
                  id="password" type={showPassword ? 'text' : 'password'} value={password}
                  onChange={e => setPassword(e.target.value)}
                  required disabled={loading} autoComplete="current-password"
                  placeholder="Enter password" className="input-base"
                />
                <button
        ...
        <p className="text-center text-sm text-theme-text-muted mt-6">
          Video Mixer © {new Date().getFullYear()}
        </p>
      </div>
    </div>
  );
